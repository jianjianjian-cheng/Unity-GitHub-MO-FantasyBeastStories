...
namespace Controllers.Character
{
    public static class CharacterAssetLoader
    {
        ...
        public static void LoadCharacterAssets(PlayerController controller, string characterName)
        {
            ...
            var attrConfig = AssetLoader.TryLoadAsset<PlayerAttributeConfigSO>($"Config/PlayerConfig/{name}Attr");
            if (attrConfig == null)
                attrConfig = AssetLoader.TryLoadAsset<PlayerAttributeConfigSO>("Config/PlayerAttributeConfig");
            if (attrConfig != null)
            ...
            if (!hasModel)
            {
                var modelPrefab = AssetLoader.TryLoadAsset<GameObject>($"Characters/{name}_Model");
                if (modelPrefab != null)
                {
            ...
            {
                var animController = AssetLoader.TryLoadAsset<RuntimeAnimatorController>($"Characters/{name}_Anim");
        ...
        public static void PreloadCharacterAssets(string characterName)
        {
            ...
            AssetLoader.TryLoadAsset<PlayerAttributeConfigSO>($"Config/PlayerConfig/{name}Attr");
            AssetLoader.TryLoadAsset<GameObject>($"Characters/{name}_Model");
            AssetLoader.TryLoadAsset<RuntimeAnimatorController>($"Characters/{name}_Anim");
        }
    }
}
