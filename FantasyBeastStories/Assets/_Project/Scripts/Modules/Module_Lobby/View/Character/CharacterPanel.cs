...
namespace UI.Lobby
{
  public class CharacterPanel : UIScreen
  {
    //角色选择面板
    [SerializeField]
    ...
    {
      screenId = "CharacterPanel";
      Debug.Log($"[CharacterPanel] Awake: screenId = {screenId}");
      base.Awake();
      // 必须在 Awake 中注册，因为 base.Awake() 会 SetActive(false)，
      // 导致 Start() 永远不会被调用（非激活对象不执行 Start）
      Debug.Log("[CharacterPanel] Awake: 正在注册到 UIManager...");
      UIManager.Instance.RegisterScreen(this);
      Debug.Log("[CharacterPanel] Awake: 注册完成");
    ...
    private void InitializeButtons()
    {
      ...
      {
        Debug.LogError("[CharacterPanel] 角色选择按钮未找到，请检查场景中是否存在 nextCharacterButton, previousCharacterButton, SwitchButton");
        return;
      }
    ...
    {
      base.OnBeforeOpen();
      Debug.Log($"[CharacterPanel] OnBeforeOpen: 面板即将打开, IsOpen={IsOpen}");
      ...
      int savedIndex = ServiceLocator.Get<SaveManager>() != null
    ...
    {
      base.OnAfterClose();
      Debug.Log($"[CharacterPanel] OnAfterClose: 面板已关闭, IsOpen={IsOpen}");
    ...
    {
      GameObject prefab = AssetLoader.LoadAsset<GameObject>("Lobby_UI_Model_" + charactorIndex);
      if (prefab == null)
      {
    ...
    private void RefreshCharacterInfo(int charactorIndex)
    {
      ...
      foreach (string desc in info.abilityDescriptions)
      {
        ...
        CharacterInfoItem item = itemObj.GetComponent<CharacterInfoItem>();
        if (item != null)
    ...
    private void SwitchCharactorButtonClicked()
    {
      ...
      {
        Debug.LogWarning($"[CharacterPanel] 未找到角色索引 {currentCharactorIndex} 对应的 Prefab 名称");
        return;
      }
