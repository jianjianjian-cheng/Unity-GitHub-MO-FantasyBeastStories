...
namespace Controllers.Character
{
    ...
    public class SpectatorCameraController : MonoBehaviour
    {
        ...
        {
            aliveTeammates = ServiceLocator.Get<PlayerManager>()?.GetAliveTeammateTransforms(localActorNumber)
                             ?? new List<Transform>();
        }
        ...
        private void HandleAllTeammatesDead()
        {
            ...
            // 延迟2秒后返回大厅
            if (ServiceLocator.TryGet<GameManager>(out var gameMgr))
            {
                gameMgr.TriggerLobbyTransition(2f);
            }
        }
