...
namespace UI.Task
{
  public class MissionPanel : UIScreen
  {
      [Header("任务面板")]
      ...
      // 运行时生成的任务项列表
      private List<MissionItem> taskItems = new List<MissionItem>();
      ...
      protected override void OnAfterClose()
      {
          ...
          // 标记任务红点已读
          ServiceLocator.Get<RedDotController>()?.MarkAsRead(RedDotKeys.MissionNew);
      }
      ...
      private void BuildTaskList()
      {
          ...
          var settleResult = ServiceLocator.Get<QuestTaskManager>().SettlePendingProgress();
          Debug.Log($"[MissionPanel] 任务结算完成：{settleResult.delta.Count} 个任务有新增进度");

          // 2. 从 Resources 加载任务数据库
          var database = AssetLoader.LoadAsset<QuestTaskDatabaseSO>("Lobby_QuestTask_QuestTaskDatabase");
          if (database == null || database.allTasks == null || database.allTasks.Count == 0)
          {
              Debug.LogWarning("[MissionPanel] 未找到 QuestTaskDatabase 或数据为空");
              return;
          }
          ...
          foreach (var taskData in database.allTasks)
          {
              ...
              var item = go.GetComponent<MissionItem>();
              if (item == null)
              {
                  Debug.LogWarning($"[MissionPanel] 预制体缺少 MissionItem 组件");
                  Destroy(go);
                  continue;
              }
              ...
              Debug.Log($"[MissionPanel] 任务[{taskData.taskId}] {taskData.taskDescription}: 旧进度={oldVal}, 新增={deltaVal}, 当前={newVal}, 目标={taskData.targetCount}");
