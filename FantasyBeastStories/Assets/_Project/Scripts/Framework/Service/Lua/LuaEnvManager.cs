...
public class LuaEnvManager
{
  ...
  {
    // Lua require 用 . 分隔模块路径，需转换为下划线（匹配 Addressable address）
    string relativePath = filePath.Replace('.', '_');

    // 通过 Addressables 加载（带 Group 前缀的地址）
    // Main.lua 在 Local Group，其余在 Lobby Group
    string prefix = filePath == "Main" ? "Local_Lua_" : "Lobby_Lua_";
    var resAsset = AssetLoader.TryLoadAsset<TextAsset>(prefix + relativePath);
    if (resAsset == null)
      resAsset = AssetLoader.TryLoadAsset<TextAsset>(prefix + relativePath + ".lua");
    if (resAsset != null)
    {
