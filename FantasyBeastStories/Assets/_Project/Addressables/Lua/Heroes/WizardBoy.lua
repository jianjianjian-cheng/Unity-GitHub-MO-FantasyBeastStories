...
function M.OnStart(player)
    ...
    if player:IsLocalPlayer() then
        ...
        if mgr then
            mgr:SetCurrentEventName(CS.Controllers.Card.CharacterCardType.WizardBoy)
        end
    end
end

function M.OnSkillQuery(player, data)
    if data.queryType == CS.Core.SharedModel.SkillQueryType.GetMaxAttackCount then
...
function M.OnSwitchElement(player, elementInt)
    ...
    if gameSettings and not gameSettings.IsTest then
        local domainRpc = CS.Core.Network.NetworkServiceLocator.DomainRpcService
        if domainRpc then
            domainRpc:InvokeRPC("RPC_InitElementPool", CS.Core.SharedModel.NetworkTarget.Others,
                CS.Core.Network.NetworkServiceLocator.ObjectService:GetViewID(player.gameObject),
                elementInt)
        end
    end
end
