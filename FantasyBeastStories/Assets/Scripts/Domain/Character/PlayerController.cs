    ...
    public class PlayerController : MonoBehaviourPun
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            EventChannelLocator.MainContainer.playerAttributeChannel.Raise(
                new PlayerAttributeData(PlayerAttributeQueryType.RegisterAttribute, EventNames.PlayerAttribute_Main, attributePlayer)
                { playerId = localActorNumber.ToString(), attributeName = EventNames.PlayerAttribute_Main }
            );
        ...
        protected virtual void MoveCharacter()
        {
            ...
            if (movement != Vector3.zero)
            {
                ...
                transform.rotation = Quaternion.Slerp(
                    transform.rotation,
                    targetRotation,
                    rotationSpeed * UnityEngine.Time.fixedDeltaTime
                );
            }
        }
        ...
        protected virtual void RPC_SyncPlayerElement(int actorNumber, int elementInt)
        {
            ...
            { playerId = actorNumber.ToString(), attributeName = EventNames.PlayerAttribute_Main };
